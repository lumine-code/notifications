...
module.exports = NotificationsLogItem = (function () {
  NotificationsLogItem = class NotificationsLogItem {
    ...
    render() {
      const notificationView = lumine.views.getView(this.notification);
      ...
      this.subscriptions.add(
        lumine.tooltips.add(this.timestamp, { title: formatAbsolute(this.timestampDate) }),
      );
    ...
    renderNotification(view) {
      ...
      if (nButtons != null) {
        for (var button of Array.from(nButtons.children)) {
          ...
          logButton.addEventListener("click", function (e) {
            if (e.currentTarget.matches('a[href]:not([href="#"])')) return;
            const newEvent = new MouseEvent("click", e);
            return e.target.originalButton.dispatchEvent(newEvent);
          });
          for (var tooltip of Array.from(lumine.tooltips.findTooltips(button))) {
            this.subscriptions.add(lumine.tooltips.add(logButton, tooltip.options));
          }
      ...
      nElement.classList.add("notifications-log-notification", this.notification.getType());
      this.iconDisposable?.dispose();
      this.iconDisposable = lumine.icons.applyTo(
        nElement,
        { name: this.notification.getIcon(), context: "notifications-log" },
        { setData: false },
      );
