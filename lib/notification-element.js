...
// The button variants the UI theme paints, and so the ones a caller can name
// in place of the colour a notification's severity would otherwise give it.
...
module.exports = NotificationElement = (function () {
  NotificationElement = class NotificationElement {
    static initClass() {
      ...
      this.prototype.removeNotificationTimeout = null;
    }
    ...
    constructor(model, visibilityDuration) {
      ...
      this.element = document.createElement("lumine-notification");
    ...
    render() {
      ...
      notificationContainer.innerHTML = lumine.tools.markdown.render(this.model.getMessage());
      ...
      if ((metaContent = options.description)) {
        ...
        description.innerHTML = lumine.tools.markdown.render(metaContent);
      }
      ...
      if (options.buttons && options.buttons.length > 0) {
        ...
        options.buttons.forEach((button) => {
          ...
          const classNames = button.className != null ? button.className.split(" ") : [];
          // The notification's own severity colours its buttons, unless the
          // caller named a variant itself — a notification offering one
          // recommended answer asks for `btn-primary` on it. Adding both would
          // leave that button wearing two variants, and which one it wore
          // would be decided by the order the active theme declared them in.
          if (!classNames.some((name) => BUTTON_VARIANTS.has(name))) {
            buttonEl.classList.add(buttonClass);
          }
          if (classNames.length > 0) {
            buttonEl.classList.add.apply(buttonEl.classList, classNames);
          }
    ...
    renderFatalError() {
      ...
      lumine.tooltips.add(copyReportButton, { title: copyReportButton.getAttribute("title") });
      copyReportButton.addEventListener("click", async (e) => {
        e.preventDefault();
        try {
          const issueBody = await this.issue.getIssueBody();
          await lumine.clipboard.write(issueBody);
          lumine.notifications.addSuccess("Error report copied to the clipboard.");
        } catch (error) {
          lumine.notifications.addWarning("Unable to copy the error report.", {
      ...
      // We only show the create issue button if it's clearly in lumine core or in a package with a repo url
      if (issueButton.parentNode != null) {
        if (packageName != null && repoUrl != null) {
          issueButton.textContent = `Open issue on the ${packageName} package`;
        } else {
          issueButton.textContent = "Open issue on lumine-code/lumine";
        }
...
`;
            ...
            const packagePath = __guard__(
              lumine.packages.getLoadedPackage(packageName),
              (x) => x.path,
            );
...
`;
            ...
            fatalNotification.innerHTML +=
              " You can help by opening an issue. Copy the error report, open GitHub, and paste the report there.";
            issueButton.setAttribute("href", this.issue.getIssueUrl());
          }
        });
      } else {
        return Promise.resolve();
    ...
    handleRemoveAllNotificationsClick() {
      ...
      for (var notification of Array.from(notifications)) {
        lumine.views.getView(notification).removeNotification();
    ...
    removeNotificationAfterTimeout() {
      if (this.element === document.activeElement) {
        lumine.workspace.getActivePane().activate();
      }

      return (this.removeNotificationTimeout = setTimeout(() => {
        this.removeNotificationTimeout = null;
    ...
    applyIcon() {
      this.iconDisposable = lumine.icons.applyTo(
        this.element,
        { name: this.model.getIcon(), context: "notifications" },
        { setData: false },
      );
    }
